...
$(document).ready(function () {
    ...
    $("#checkaddress").click(function (event) {
       ...
       event.stopImmediatePropagation();
        ...
        if ($('#mySelect :selected').val() == "KOREA, REPUBLIC OF" || $('#mySelect :selected').val()=="VIET NAM") {
    ...
    function UpdateMarkerCustomer(addressBuilder2, addressType, postCode) {
        ...
        $.getJSON('https://maps.googleapis.com/maps/api/geocode/json?address=' + encodeURIComponent(addressBuilder2) + '&sensor=false&libraries=places&language=' + language + '', function (data) {
    ...
    function checkAddress(Address) {
        ...
        //else if (Address.country == "VIET NAM")
        //{
        //    lang = 'vi-VI';
            //}
        else if (Address.country == "VIET NAM" || Address.country == "VIETNAM" || Address.country == "Vietnam") {
...
//Modify Address fro part Delivery type if already address is available 
        ...
        if (addr3[0].length > 0) {
            for (var i = 0; i < addr3[addr1.length - 1].length; i++) {
                ...
                //else if (addr3[0][i][4] == "VIET NAM") {
                //    language = 'vi-VI';
                    //}
                else if (addr3[0][i][4] == "VIET NAM" || addr3[0][i][4] == "VIETNAM" || addr3[0][i][4] == "Vietnam") {
                    addr3[0][i][4] = "Vietnam";
                    language = 'vi';
                }
                else {
        ...
        if (Address.addressType != 'Customer Site' &&   $("#countdata").text()=="Creation") {
            ...
            if (Address.addressType == 'Drop Point' || Address.addressType == 'Warehouse') {
                $.getJSON('https://maps.googleapis.com/maps/api/geocode/json?address=' + encodeURIComponent(addressBuilder) + '&sensor=false&libraries=places&language=' + lang + '', function (data) {
                    if (data.results.length > 0) {
            ...
            if (Address.addressType == 'FE Address' || Address.addressType == 'GE Office' || Address.addressType == 'Other') {
                ...
                $.getJSON('https://maps.googleapis.com/maps/api/geocode/json?address=' + encodeURIComponent(addressBuilder) + '&sensor=false&libraries=places&language=' + lang + '', null, function (data) {
                    if (data.results.length > 0) {
